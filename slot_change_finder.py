...
def main():
    ...
    if not code:
       ...
       t0 = time.time()
    try:
        _ = storage_at(str(w3.provider.endpoint_uri), address, slot, lo)
    except Exception as e:
        print(f"❌ Cannot read baseline at {lo}: {e}", file=sys.stderr)
        sys.exit(2)
    ...
    end_val  = storage_at(str(w3.provider.endpoint_uri), address, slot, hi)
        if len(base_val) != 32 or len(end_val) != 32:
        print("❌ Storage read not 32 bytes.", file=sys.stderr)
        sys.exit(2)
